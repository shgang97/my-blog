...
func GetPostTotal1(slug string) int {
	var row *sql.Row
	if slug == "" {
...
func GetPostsByPage(page int, pageSize int) ([]models.Post, error) {
	page = (page - 1) * pageSize
	var err error
	var rows *sql.Rows
	if err != nil {
	...
	for rows.Next() {
		var post models.Post
		err := rows.Scan(&post.Id, &post.UserId, &post.Title, &post.Description, &post.Content, &post.CreateTime, &post.UpdateTime, &post.Status)
		if err != nil {
			log.Println("failed to convert to Post:", err)
			return nil, err
		}
